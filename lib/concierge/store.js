...
const mapInquiry = row => ({ id: row.id, contactId: row.contact_id, name: row.name, email: row.email, company: row.company, message: row.message, status: row.status, createdAt: timestamp(row.created_at), serviceIds: row.service_ids || [], consent: row.consent_events || [] })
  ...
  return {
    ...
    async listInquiries(limit) {
      const found = rows(await db.execute(sql`
        SELECT i.*, c.name, c.email, c.company,
          COALESCE((SELECT json_agg(service_id ORDER BY service_id) FROM legacy_inquiry_interests WHERE inquiry_id=i.id), '[]'::json) AS service_ids,
          COALESCE((SELECT json_agg(e ORDER BY e.created_at,e.id) FROM legacy_consent_events e WHERE e.inquiry_id=i.id), '[]'::json) AS consent_events
