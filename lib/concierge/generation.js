...
export function createGeneration(env = process.env) {
  const gateway = createAppGateway(env)
  function config() {
    if (!env.VERCEL && !env.AI_GATEWAY_API_KEY && !env.VERCEL_OIDC_TOKEN) throw new HttpError(503, 'The assistant is not connected yet. You can still review services and prepare an inquiry.')
    return { model: gateway(env.CONCIERGE_MODEL || model), maxOutputTokens: 1800, maxRetries: 0, timeout: 45000 }
  }
  return {
    async identify(text) {
