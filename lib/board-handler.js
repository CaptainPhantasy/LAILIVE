...
6. No false harmony. If the board reaches 5-0 agreement, something went wrong.`
  ...
  async function POST(req) {
    ...
    // The AI SDK automatically prefers AI_GATEWAY_API_KEY and uses Vercel OIDC
    // when deployed without an explicit key. Keep auth implicit so OIDC is not
    // accidentally passed as an API key override.
    ...
    try {
      ...
      return new Response(stream, {
        ...
        headers: {
          ...
          'X-Board-Auth': env.AI_GATEWAY_API_KEY ? 'api-key' : 'oidc',
        },
      })
    } catch (error) {
      clearTimeout(timeoutId)
