...
`dist/robots.txt` welcomes search and AI crawlers while keeping `/owner/` and `/api/` out. `dist/sitemap.xml`, `dist/llms.txt` and `dist/llms-full.txt` are generated by `npm run build:seo` from `dist/service-catalog.json` — run it after catalog or page edits. Each public page carries its own title, description, canonical URL, social card (`dist/og-solutions.jpg`) and JSON-LD structured data; `/solutions/` embeds the full 54-service list between the `seo:services-jsonld` markers. `test/seo.test.js` guards all of it.
